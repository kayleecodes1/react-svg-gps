import { SVG, registerWindow, type Dom } from '@svgdotjs/svg.js';
...
const renderNestedCircleElement = (levels: number, step: StepConfig) => {
    const root = SVG();
    let currentNode: Dom = root;
    const currentPosition = { x: 0, y: 0 };
    let currentScale = 1;
    for (let i = 0; i < levels; i++) {
        const group = root.group().attr({
            transform: `translate(${step.translateX},${step.translateY}) scale(${step.scale})`,
        });
    ...
    const circle = root
        .circle()
        .attr({ cx: step.translateX, cy: step.translateY, r: 1 });
...
it('calculates the correct global point', () => {
    const result = renderNestedCircleElement(5, {
        translateX: 10,
        translateY: 10,
        scale: 2,
    });
    ...
    testAnchors.forEach((anchor) => {
        testOffsets.forEach((offset) => {
            ...
            expect(point).toEqual({
                x:
                    globalPosition.x +
                    (2 * anchor.x - 1) * globalScale +
                    offset.x * globalScale,
                y:
                    globalPosition.y +
                    (2 * anchor.y - 1) * globalScale +
