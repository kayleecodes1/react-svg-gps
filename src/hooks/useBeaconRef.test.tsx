...
jest.mock('../utilities/getGlobalPoint');
jest.mock('../models/BeaconStore');
jest.mock('../components/BeaconProvider');

const renderUseBeaconRef = (callback: () => any, beaconStore: BeaconStore) =>
    renderHook(callback, {
        wrapper: ({ children }) => (
...
it('registers on mount and unregisters on unmount', () => {
    ...
    const { unmount } = renderUseBeaconRef(
        () => useBeaconRef({ id: 'A' }),
        beaconStore,
    );
...
it('reports its position on each render', () => {
    const beaconStore = new BeaconStore();
    const { rerender } = renderUseBeaconRef(
        () => useBeaconRef({ id: 'A' }),
        beaconStore,
    );
    for (let i = 0; i < 9; i++) {
    ...
    for (let i = 0; i < 10; i++) {
        expect(
            (beaconStore.reportBeaconPoint as jest.Mock).mock.calls[i][0],
        ).toEqual('A');
    }
});
...
it('reports a position if there is a ref', () => {
    const beaconStore = new BeaconStore();
    const { result, rerender } = renderUseBeaconRef(
        () => useBeaconRef({ id: 'A' }),
        beaconStore,
    );
    ...
    rerender();
    expect(beaconStore.reportBeaconPoint).toHaveBeenNthCalledWith(2, 'A', {
        x: 50,
        y: 100,
    });
});
...
it('propagates `anchor` and `offset` values', () => {
    ...
    const { result, rerender } = renderUseBeaconRef(
        () => useBeaconRef({ id: 'A', anchor, offset }),
        beaconStore,
    );
    const ref = result.current;
    ref.current = Symbol('SVGGraphicsElement');
