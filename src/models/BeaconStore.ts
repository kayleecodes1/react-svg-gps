export default class BeaconStore {
    ...
    public registerBeacon(id : string) : void {
        if (this.points.has(id)) {
            console.warn('BeaconStore: Attempt to register duplicate beacon ID `%s`', id);
            return;
        }
    ...
    public unregisterBeacon(id : string) : void {
        if (!this.points.has(id)) {
            console.warn('BeaconStore: Attempt to unregister unregistered beacon ID `%s`', id);
            return;
        }
    ...
    public reportBeaconPoint(id : string, point : Point | null) : void {
        if (!this.points.has(id)) {
            console.warn('BeaconStore: Attempt to report unregistered beacon ID `%s`', id);
            return;
        }
